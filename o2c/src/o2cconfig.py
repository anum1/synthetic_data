...
(supply chain) and GlobalTech (HR) datasets, so all four anchor to the same
as-of convention: the last day of the previous complete month. Event timing is
expressed in months relative to that date, never as absolute dates, so
regenerating next year still tells this year's story.
"""
...
class Scenario:
    ...
    def fulfillment(self) -> dict:
        return self.cfg["fulfillment"]

    @property
    def shipping(self) -> dict:
        return self.cfg["shipping"]

    @property
    def billing(self) -> dict:
        return self.cfg["billing"]
    ...
    def tier_scale(self) -> float:
        ...
        """
        full = float(self.cfg["tiers"]["full"]["orders_per_month_base"])
        return float(self.sizes["orders_per_month_base"]) / max(full, 1.0)
    ...
    def scaled(self, share: float, of: str = "customers") -> int:
...
def _resolve_timeline(cal: dict) -> Timeline:
    ...
    if anchor == "fixed":
        ...
        # never shows up as a fake collapse in the bookings trend.
